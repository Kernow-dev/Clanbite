...
| `clanbite.notifications.polled` | Fired after each successful HTTP poll response or WebSocket message, or from extensions after related mutations (e.g. Points click award). Payload includes `transport` (`http`, `websocket`, or `synthetic`), plus `response`, `payload`, or `reason` as applicable. Use to sync other UI on the same cadence as the bell. |
